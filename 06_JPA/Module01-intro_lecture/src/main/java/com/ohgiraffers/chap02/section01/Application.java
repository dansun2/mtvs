...
* 영속성 컨텍스트의 역할: EntityManager는 영속성 컨텍스트를 통해 엔티티를 관리한다.
* 영속성 컨텍스트는 동일한 엔티티(예: 동일 roleId를 가진 Role 객체)를 단일 인스턴스로 관리하는 "1차 캐시" 역할을 한다.
* 동일성 보장: em.getReference(Role.class, 1)를 두 번 호출하여 role과 role2를 조회하면,
* 영속성 컨텍스트가 동일한 roleId에 대해 동일한 인스턴스를 반환하므로 role == role2가 true로 평가된다.
...
* 예를 들어, role 인스턴스를 수정하면 동일한 roleId를 가진 role2도 동일한 인스턴스를 참조하므로,
...
* role == role2가 true로 평가되어 동일 데이터가 동일한 인스턴스로 관리됨.
...
* EX) 동일 권한 비교: JPA를 사용하면 role == role2가 true로 평가되어 비교가 성공.
...
public class Application {
    ...
    static {
        ...
        /*
        * EntityManagerFactory는 영속성 유닛(Persistence Unit)을 기반으로
        * EntityManager를 생성해주는 팩토리 클래스이다.
        *
        * 내부적으로 데이터베이스 커넥션 풀, JPA 설정 정보, 엔터티 메타데이터 등을 기반으로 JPA 환경을 초기화하며,
        * 한 번 생성한 후 애플리케이션이 종료될때까지 재사용한다.
        * - 비용이 크기 때문에 반드시 앱 전체에서 하나만 생성해야 한다.
        * */
    ...
    public static void main(String[] args) {
        /*
        * EntityManager는 실제 DB 작업(조회, 저장, 수정, 삭제)을 담당하는 객체이다.
        * 내부적으로 영속성 컨텍스트를 생성하여 엔티티를 관리하고 트랜잭션 단위로 동작한다.
        * 엔티티를 관리한다는 것은 메모리 상에서 객체 상태 변화를 추적하고, 트랜잭션 커밋 시 DB 반영 여부를 자동으로 결정하는 기능까지 포함한다.
        * EntityManager는 쓰레드 간 공유를 해서는 안된다.
        * */
