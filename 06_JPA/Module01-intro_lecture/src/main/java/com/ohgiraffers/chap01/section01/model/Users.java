...
* 예를 들어 "User" 객체가 특정 역할(Role)을 가진다고 할 때, 객체지향에서는 "Role role"과 같은 필드를 추가한다.
*
...
* 이 차이는 객체지향과 RDB의 패러다임의 차이를 보여준다.
*
* 이러한 차이로 인해 설계와 구현에서 혼란이 발생할 수 있다.
* - 객체 지향 관점 : "User" 객체는 "Role" 객체를 직접 참조하므로, "user.getRole().getRoleName()"과 같은 방식으로
*                  역할 이름을 쉽게 가져올 수 있다.
...
* 'roleId'는 'Role' 객체를 나타내는 참조가 아니라 단순한 외래키값에 불과하기 때문에, 객체지향 설계의 캡슐화와 객체 간 협력 원칙에 어긋날 수 있다.
*
* 이 차이는 특정 이벤트의 주체와 동작을 이해하는데 혼란을 초래할 수 있다.
* ex) 사용자 역할 변경
*   - 객체지향에서는 'user.setRole(newRole)'
