...
/*
* 객체지향(OOP)와 RDB의 패러다임 차이 DTO 사용으로 인한 문제
*
* 이 "UserRoles" 클래스는 객체 지향 프로그래밍에서 RDB의 데이터를 표현하기 위해 설계된 DTO(Data Transfer Object)이다.
...
*
* 따라서, 객체지향에서는 'User'와 'Role'을 별도의 객체로 나누어서 설계하지만,
...
* - RDB 관점 : 조인 쿼리로 데이터를 평면화하여 반환하므로, UserAndRole DTO에 roleName을 직접 포함시켜야 한다.
*   이로 인해 객체지향의 캡슐화 원칙이 약화된다.
*   DTO를 사용하면 'User'와 'Role'의 관계가 단순한 데이터로 평면화되어, 객체지향적인 설계의 장점(예 : 객체 간 협력, 캡슐화)가 손실된다.
