...
*
* 다형성의 필요성
* - 코드의 중복을 줄이고, 유지보수를 쉽게 하며, 다양한 객체를 유연하게 다룰 수 있게 해준다.
*
* 다형성을 사용하지 않으면 생기는 문제
* 1) 코드 중복 :
*   - 각 동물 타입마다 별도 변수와 메서드 호출을 작성해야 한다.
*   - 예 : Dog dog = new Dog();
*          Cat cat = new Cat(); 처럼 각각 선언하고 호출하면 코드가 길어지게 된다.
*
* 2) 유지보수 어려움
*   - 새 동물(예 : Bird)가 추가될 때 마다 새로운 변수와 호출 코드를 추가해야 한다.
*   - 예 : Bird bird = new Bird(); bird.makeSound()를 또 작성해야됨.
*
* 3) 유연성 부족
*   - 동물을들 한 번에 묶어서 다룰 방법이 없다.
*   - 예 : "모든 동물 소리 내라" 와 같이 통합 명령이 불가능하다.
...
*       2) 오버라이딩 된 makeSound()가 각 객체에 맞게 실행됨
*   - 장점 :
*       1) 중복 제거 : 한 번의 반복문으로 모든 동물 호출
*       2) 유지 보수 쉬움 : 새 동물 추가 시 배열에 넣기만 하면 끝
