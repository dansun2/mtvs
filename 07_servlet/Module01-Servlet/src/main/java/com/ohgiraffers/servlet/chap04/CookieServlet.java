...
/*
* 쿠키
* - 쿠키는 클라이언트와 서버 간에 상태 정보를 유지하기 위해 사용되는 작은 데이터 조각이다. (서버가 클라이언트한테 보내주는거) 세션스토리지 이런건 클라이언트측에서?
...
public class CookieServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (cookieValue != null && !cookieValue.isEmpty()) {
            ...
            *
            * - 동작
            *   1. 서버에서 "cooke" 객체를 생성하고 "HttpServletResponse.addCookie()"로 클라이언트에 전송
            *   2. 클라이언트가 다음 요청 시 "Cookie" 헤더에 쿠키를 포함해서 서버로 전달함
            *   3. 서버는 "HttpServletRequest.getCookies()"로 쿠키 배열을 받아 필요한 쿠키를 추출
            *
            * - 주의
            *   - 쿠키 값은 문자열로만 저장되며, 특수문자(예: 세미콜론, 쉼표 등)는 제거하거나 인코딩 필요
            *   - 쿠키 제한(약 4kb)이 있으므로 대용량 데이터에는 적합하지 않음
            * */
