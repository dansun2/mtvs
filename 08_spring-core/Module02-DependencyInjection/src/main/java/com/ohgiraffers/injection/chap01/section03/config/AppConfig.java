...
import com.ohgiraffers.injection.chap01.section03.service.PaymentService;
...
public class AppConfig {
    ...
    *   - 실무 팁: @Qualifier는 디버깅 시 주입된 Bean을 추적하기 쉬워 유지보수에 유리. -> 낮은 레벨의 회사에 가면 Qualifier를 잘 안씀 Autowired로 때려넣는다.
    * */
    @Bean
    public PaymentService naverPayService(
            @Qualifier("naverPayGateway") PaymentInterface paymentInterface) { // @Qualifier 는 bean의 이름을 명시적으로 지정
        return new PaymentService(paymentInterface);
    }
}
