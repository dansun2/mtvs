...
*
* 동작 과정: -> Bean간의 충돌이 있을 수 있어서 Lazy옵션이 있는데 쓰는걸 별로 권장하지 않음 -> 애초에 클래스간에 싸이클이 돌면 설계가 잘못되었다고 볼 수 있다
...
*/
@Configuration
//basePackages는 어디서부터 component를 찾을건지 뜻함. 기본 경로는 section01/config이다 (@Configuration때문인가?)
